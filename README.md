...
or filters cannot apply to the selected object and it contains exactly one
array child, the query selects that collection and reports its full pointer as
`response_meta.inferred_response_path`; ambiguous objects still require a more
specific path. The full serialized MCP result is capped at
