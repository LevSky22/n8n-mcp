...
- n8n_test_workflow - Run a workflow over HTTP triggers or through n8n's instance-level MCP server
...
- n8n_workflow_versions - Local or native n8n version history, diff and rollback
- n8n_deploy_template - Deploy templates directly to n8n instance
- n8n_manage_datatable - Manage data tables, rows and columns
...
- n8n_manage_agents - Manage n8n Agents through n8n's instance-level MCP server
- n8n_explore_node_resources - Resolve dynamic node resource options using a real credential
- n8n_list_catalog - List instance-level projects or tags with official-MCP fallback
...
- query_response_artifact - Query structured JSON inside an artifact with strict camelCase arguments. Start with describe=true, then filter, project, or use bounded literal textSearch. pageSize is 1-100; continue with responseMeta.nextCursor as cursor
